...
class Crypt {
    ...
    constructor(key) {
        ...
        for (let i = 0; i < spin; i++)
            rng.random_int();
        const keyBuff = new bireader.BiWriter(Buffer.alloc(32));
        const iv = new bireader.BiWriter(Buffer.alloc(16));
        for (let i = 0; i < 8; i++) {
...
class JPDecode extends JPBase {
    ...
    checkFilePath(filePath) {
        ...
        biTest.close();
        biTest = new bireader.BiReader(testBuffer);
    ...
    reinitializeState() {
        if (this.useStream) {
            ...
            this.compReader = new bireader.BiReaderStream(this.fileName);
            ...
            if (this.Encrypted) {
                ...
                const compWriter = new bireader.BiWriterStream(this.fileName + ".comp");
                ...
                compWriter.close();
                this.compReader = new bireader.BiReaderStream(this.fileName + ".comp");
                this.compReader.endian = this.endian;
            ...
            if (this.Compressed) {
                ...
                if (this.tempCreated) {
                    ...
                    const tempcompWriter = new bireader.BiWriterStream(this.fileName + ".comp.tmp");
                    ...
                    compWriter.trim();
                    compWriter.close();
                    const compReader = new bireader.BiReaderStream(this.fileName + ".comp");
            ...
            if (this.tempCreated) {
                ...
                this.compReader.open();
                this.valueReader = new bireader.BiReaderStream(this.fileName + ".comp");
                this.strReader = new bireader.BiReaderStream(this.fileName + ".comp");
                ...
                this.compReader.open();
                this.valueReader = new bireader.BiReaderStream(this.fileName);
                this.strReader = new bireader.BiReaderStream(this.fileName);
                this.valueReader.fd = this.compReader.fd;
                this.valueReader.endian = this.compReader.endian;
                this.valueReader.size = this.compReader.size;
                this.valueReader.sizeB = this.compReader.sizeB;
            ...
            this.compReader = new bireader.BiReader(decomBuffer);
            this.compReader.endian = this.endian;
            if (this.Encrypted) {
            ...
            this.strReader = new bireader.BiReader(this.compReader.extract(Number(this.STR_SIZE), true));
...
class JPEncode extends JPBase {
    ...
    encode(object, filePath) {
        ...
        try {
            ...
            if (!this.useStream) {
                ...
                const fileFile = new bireader.BiWriterStream(this.fileName);
    ...
    reinitializeState() {
        if (this.useStream) {
            this.valueWriter = new bireader.BiWriterStream(this.fileName + ".values", { extendBufferSize: 2048 });
            this.valueWriter.endian = this.endian;
            this.strWriter = new bireader.BiWriterStream(this.fileName + ".strings", { extendBufferSize: 2048 });
            this.strWriter.endian = this.endian;
        }
        else {
            this.valueWriter = new bireader.BiWriter(Buffer.alloc(2048), { extendBufferSize: 2048 });
            this.valueWriter.endian = this.endian;
            this.strWriter = new bireader.BiWriter(Buffer.alloc(2048), { extendBufferSize: 2048 });
            this.strWriter.endian = this.endian;
        }
    }
    ;
    ...
    encodeSymbol(valueWriter, object) {
        const extBuffer = new bireader.BiWriter(Buffer.alloc(512));
    ...
    buildHeader(endian) {
        ...
        const bw = new bireader.BiWriter(Buffer.alloc(this.HEADER_SIZE));
    ...
    encrypt(EncryptionExcluded, Encryptionkey) {
        ...
        if (!this.useStream) {
            ...
            this.compWriter = new bireader.BiWriter(compBuffer);
    ...
    compress() {
        ...
        if (!this.useStream) {
            ...
            const tempcompWriter = new bireader.BiWriterStream(temp);
            tempcompWriter.open();
